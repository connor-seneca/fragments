...
// Delete a fragment's metadata and data from an S3 Bucket
...
async function deleteFragment(ownerId, id) {
  logger.info(`Inside deleteFragment with ownerId:${ownerId} : id:${id}`);
  ...
  try {
    await Promise.all([s3Client.send(commandS3), ddbDocClient.send(commandDB)]);
  } catch (err) {
    logger.error({ err }, 'Error inside deleteFragment aws model/data');
    throw err;
  }
}
